  ...
  function AdaptateurLocal(cfg) {
    ...
    return {
      ...
      reserver: function (d) {
        ...
        var conflit = liste.some(function (r) {
          return r.statut !== 'annule' && r.jour === d.jour &&
                 deb < str2min(r.fin) && fin > str2min(r.debut);
        });
        if (conflit) return Promise.resolve({ ok: false, erreur: 'creneau_pris' });
  ...
  function creneauxDuJour(cfg, etat, date, prestation) {
    ...
    plages.forEach(function (p) {
      ...
      for (var t = deb; t + prestation.duree <= fin; t += cfg.etablissement.pasMinutes) {
        ...
        var chevauche = occupes.some(function (o) {
          return t < str2min(o.fin) && (t + prestation.duree) > str2min(o.debut);
        });
